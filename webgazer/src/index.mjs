...
async function getPrediction(regModelIndex) {
  var predictions = [];
  var leftEyePrediction = [];
  var rightEyePrediction = [];
  ...
  for (var reg in regs) {
    leftEyePrediction.push(regs[reg].predictOneEye(latestEyeFeatures, true));
    rightEyePrediction.push(regs[reg].predictOneEye(latestEyeFeatures, false));
  }
  for (var reg in regs) {
  ...
  if (regModelIndex !== undefined) {
    return predictions[regModelIndex] === null ? null : {
      ...
      'gazeLeft': leftEyePrediction[regModelIndex],
      'gazeRight': rightEyePrediction[regModelIndex],
    ...
    return predictions.length === 0 || predictions[0] === null ? null : {
      ...
      'y' : predictions[0].y,
      'eyeFeatures': latestEyeFeatures,
      'gazeLeft': leftEyePrediction[0],
      'gazeRight': rightEyePrediction[0],
      'all' : predictions
    };
  }
}
