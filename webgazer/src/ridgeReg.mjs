...
reg.RidgeReg.prototype.predict = function(eyesObj) {
  ...
  // console.log(`hello from predict`);
  ...
  // console.log(`coefficientsX: ${JSON.stringify(coefficientsX)}`);
  // console.log(`coefficientsY: ${JSON.stringify(coefficientsY)}`);
  ...
  // console.log(`prediction x: ${predictedX}, y: ${predictedY}`);
...
/**
 * Try to predict coordinates from one pupil data
 * after apply linear regression on data set
 * @param {Object} eyesObj - The current user eyes object
 * @param {Boolean} isLeft - is Left Eye
 * @returns {Object}
 */
 reg.RidgeReg.prototype.predictOneEye = function(eyesObj, isLeft) {
  ...
  var eyeFeats = util.getSpecificEyeFeats(eyesObj, isLeft);
  ...
  if (params.applyKalmanFilter) {
      // Update Kalman model, and get prediction
      var newGaze = [predictedX, predictedY]; // [20200607 xk] Should we use a 1x4 vector?
      newGaze = this.kalman.update(newGaze, true);
